...
import botlogo from "../chatbot.png";
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      getAnswer(respid,prompt,modelParams,authheader)
        .then(data => {
            console.log(data.bot.message.content);
            //save conversations
            setConversations(prev=>[...prev,data.bot.message.content]);
            setLoading(false);
            setmsgItems((prev) => [...prev,{ id: generateUniqueId(), who:BOTNAME, text: data.bot.message.content.trimStart()}]
            );
        }).catch(err =>{ 
            console.table(err);
            setLoading(false);　
